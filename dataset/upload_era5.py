...
Years = {
    ...
    'valid': ['2017-01-01 00:00:00', '2017-12-31 23:00:00'],
...
class era5_128x256(Dataset):
    def __init__(self, data_dir='s3://era5_np128x256', split='train', out_var=8, **kwargs) -> None:
        super().__init__()
        # print("init begin")
...
if __name__ == "__main__":
    dataset = era5_128x256(split='all',out_var=3)

    new_url = 'cluster2:s3://zwl2/era5_processed'

    for i in tqdm(range(dataset.__len__())):
        data_item = dataset.__getitem__(i)
        buffer = io.BytesIO()
        path = f"{new_url}/{i}.npy"
        np.save(buffer, data_item)
        # 获取保存后的字节数据
