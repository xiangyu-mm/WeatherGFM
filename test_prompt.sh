srun -p ai4earth -n 1 --ntasks-per-node=1 --gres=gpu:1 --job-name=xiangyu --cpus-per-task=1 torchrun \
--master_port=4387 test_prompt.py \
...
--save_ckpt_freq 1 \
--ckpt /mnt/petrelfs/zhaoxiangyu1/code/weather_prompt_new/experiments/large_finetune/dblur_version/checkpoint-4.pth \
--output_dir experiments/prompt_effect/ \
